...
suite('Nessus Parse Tests', function(){
  ...
  test("Nessus Entry 1", function(){
    ...
    var nessusStream = (new NessusStream()).stream
    nessusStream.parseNessusResult(data, function(err, out) {
      if ( err )
        throw err
      assert(obj["port"] === -1)
