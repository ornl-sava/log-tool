...
Tests for other modules go in their own test dir (eg some_module and some_module/bin go in some_module/test)
Note: this is only required so that the modules' tests can be invoked by logtool's tests.

Any needed data for these tests is in ./test/data, any other modules can include their test data where needed (some_module/test/data, or other)
