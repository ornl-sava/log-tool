...
var sendLogRecord = function (name, event) {
  if (envy.config.eventQueueServerType === 'redis')
    ...
    publishRedis({'hostname': hostname, 'timestamp': Date.now(), 'name': name, 'event': event})
}
...
/*
  ...
  Data sent to redis:
    key: sensor:destip where ip is the destination ip (or only ip if not flow)
...
var sendRedis = function (record) {
  var key = record.name + ':' + record.event.destIP
    , val = {}
  val[record.event.timestamp.toString()] = record.event
  console.log('send to redis: ' + key + '  ' + record.event.timestamp.toString())
...
var publishRedis = function (record) {
  var channel = 'events'
    , message = {}
    , key = record.name + ':' + record.event.destIP
    , val = {}
  val[record.event.timestamp.toString()] = record.event
  message[key] = val
  console.log('publish to redis channel: ' + channel + ', message: ' + util.inspect(message))
