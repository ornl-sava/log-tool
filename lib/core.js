...
module.exports.start = function () {
  ...
  for(var i=0; i<connectionConfig.length; i++){
    ...
    var currInput = inputModule.start(inputConfig[src])
    ...
    var currOutput = outputModule.start(outputConfig[dst])
    ...
    //NB: slightly different from above.  //TODO how to pass opts?
    var currParser = es.through(parserModule.data, parserModule.end)

    //TODO double check this is ok for pause/resume, or else maybe use es.connect
    util.pump(currParser, currOutput)  //hook parser -> output
    util.pump(currInput, currParser)  //hook input -> parser
  }
