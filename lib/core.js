...
function LogTool(opts) {
  ...
  */
  self.stop = function () {
    self.logger.log('info', 'Stopping...')

    self.logger.log('debug', 'Stopping parser streams')
    for(var i=0; i<parserStreams.length; i++){
    ...
    self.emit('done')
    //if(self.opts) //only do this if you were invoked from CLI, not if invoked from other node code.  //TODO move this exit() to ../logtool.js 
      //process.exit(0) 
  }
  ...
  self.logger.log('info', 'Started.  Connecting module instances...')
  ...
  if(connectionConfig.length === 0){
    ...
    for(var i=0; i< inputStreams.length; i++){
      inputStreams[i].on("error", function(msg){
        self.logger.log('error', msg)
        console.log(msg)
      } );
    }
    for(var i=0; i< outputStreams.length; i++){
      outputStreams[i].on("error", function(msg){
        self.logger.log('error', msg)
        console.log(msg)
      } );
    }
    for(var i=0; i< parserStreams.length; i++){
      parserStreams[i].on("error", function(msg){
        self.logger.log('error', msg)
        console.log(msg)
      } );
    }
